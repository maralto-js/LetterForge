...
public class LetterManager {
    ...
    public LetterManager(LetterForge plugin, StorageProvider storage) {
        this.plugin = plugin;
        this.storage = storage;
        this.ioExecutor = Executors.newSingleThreadExecutor(r -> {
            Thread t = new Thread(r, "letterforge-db");
            t.setDaemon(true);
            return t;
        });
    ...
    public void onPlayerJoin(Player player) {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            long unreadDirect = inbox.stream().filter(l -> !l.read() && !l.type().isBroadcastLike()).count();
    ...
    /**
     * Publica um jornal: calcula a edição (MAX+1) no mesmo thread de IO antes do INSERT e entrega
     * a todos online como broadcast. O id da carta é preservado, então anexos de imagem casam.
     */
    public CompletableFuture<Boolean> sendJornal(Letter letter) {
        return CompletableFuture.supplyAsync(() -> {
            Letter withEdition = letter.withEdition(storage.nextJornalEdition());
            if (!storage.insertLetter(withEdition)) return false;
            Bukkit.getScheduler().runTask(plugin, () -> {
                for (Player p : Bukkit.getOnlinePlayers()) {
                    addToCache(p.getUniqueId(), withEdition);
                    notifyNewLetter(p, withEdition);
                }
            });
            return true;
        }, ioExecutor);
    ...
    public CompletableFuture<List<Letter>> getJornaisAsync(int limit) {
        return CompletableFuture.supplyAsync(() -> storage.getJornais(limit), ioExecutor);
    }

    public CompletableFuture<Optional<Letter>> getJornalByEditionAsync(int edition) {
        return CompletableFuture.supplyAsync(() -> storage.getJornalByEdition(edition), ioExecutor);
    }
    ...
    private void notifyNewLetter(Player player, Letter letter) {
        ...
        String tag = switch (letter.type()) {
            ...
            case JORNAL    -> msgs.raw("tags.jornal");
            default        -> "";
        };
        NamedTextColor tagColor = switch (letter.type()) {
            ...
            case JORNAL    -> NamedTextColor.AQUA;
