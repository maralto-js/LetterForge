...
public interface StorageProvider {
    ...
    /** Próximo número de edição do jornal (MAX(edition)+1). */
    int nextJornalEdition();

    /** Últimos jornais publicados (não expirados), ordenados por edição desc. */
    List<Letter> getJornais(int limit);

    /** Jornal por número de edição, se ainda existir. */
    Optional<Letter> getJornalByEdition(int edition);
