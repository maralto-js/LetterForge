package com.letterforge.storage;

import com.google.gson.Gson;
import com.google.gson.reflect.TypeToken;
import com.letterforge.image.MapImageEntry;
...
public class SQLiteStorage implements StorageProvider {
    ...
    public synchronized void init() throws Exception {
        Class.forName("com.letterforge.libs.sqlite.JDBC");
        ...
        createTables();
        applySchemaVersion();
        logger.info("[LetterForge] Banco de dados inicializado (schema v" + SCHEMA_VERSION + ").");
    }
    ...
    private void createTables() throws SQLException {
        try (Statement s = connection.createStatement()) {
            ...
            try {
                s.execute("ALTER TABLE letters ADD COLUMN edition INTEGER NOT NULL DEFAULT 0");
            } catch (SQLException ignored) {}
    ...
    private void applySchemaVersion() throws SQLException {
        ...
        if (current < 3) {
            ...
            try (PreparedStatement ps = connection.prepareStatement("""
                ...
                if (fixed > 0)
                    logger.info("[LetterForge] Migracao v3: retencao de " + fixed
    ...
    public synchronized boolean insertLetter(Letter letter) {
        String sql = """
                INSERT INTO letters(id, sender_uuid, sender_name, recipient_uuid, recipient_name,
                                    pages, sent_at, deliver_at, read, delivered, type, priority, expires_at, edition)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            ps.setInt(14, letter.edition());
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            logger.severe("[LetterForge] Erro ao inserir carta: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized List<Letter> getInbox(UUID playerUUID) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao carregar inbox: " + e.getMessage());
    ...
    public synchronized List<Letter> getInboxAdmin(UUID playerUUID) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao carregar inbox (admin): " + e.getMessage());
            return Collections.emptyList();
        }
    }
    ...
    public synchronized List<Letter> getPendingDirectLetters(UUID playerUUID, long currentTime) {
        String sql = """
                ...
                WHERE recipient_uuid = ?
                  AND delivered = 0
                  AND deliver_at <= ?
                  AND type NOT IN ('BROADCAST','JORNAL')""";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, playerUUID.toString());
            ps.setLong(2, currentTime);
            return mapAll(ps.executeQuery());
        } catch (SQLException e) {
            logger.severe("[LetterForge] Erro ao buscar pendentes: " + e.getMessage());
            return Collections.emptyList();
        }
    }
    ...
    public synchronized List<Letter> getPendingBroadcasts(UUID playerUUID) {
        ...
        String sql = """
                SELECT * FROM letters
                WHERE type IN ('BROADCAST','JORNAL')
                  AND (expires_at = 0 OR expires_at > ?)
                  AND id NOT IN (
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao buscar broadcasts: " + e.getMessage());
            return Collections.emptyList();
        }
    }

    @Override
    public synchronized int nextJornalEdition() {
        String sql = "SELECT COALESCE(MAX(edition), 0) + 1 FROM letters WHERE type = 'JORNAL'";
        try (Statement s = connection.createStatement();
             ResultSet rs = s.executeQuery(sql)) {
            return rs.next() ? rs.getInt(1) : 1;
        } catch (SQLException e) {
            logger.severe("[LetterForge] Erro ao calcular edicao do jornal: " + e.getMessage());
            return 1;
        }
    }

    @Override
    public synchronized List<Letter> getJornais(int limit) {
        long now = System.currentTimeMillis();
        String sql = """
                SELECT * FROM letters
                WHERE type = 'JORNAL'
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao buscar jornais: " + e.getMessage());
            return Collections.emptyList();
        }
    }

    @Override
    public synchronized Optional<Letter> getJornalByEdition(int edition) {
        long now = System.currentTimeMillis();
        String sql = """
                SELECT * FROM letters
                WHERE type = 'JORNAL' AND edition = ?
                  AND (expires_at = 0 OR expires_at > ?)
                LIMIT 1""";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, edition);
            ...
            logger.severe("[LetterForge] Erro ao buscar jornal por edicao: " + e.getMessage());
    ...
    public synchronized void markBroadcastSeen(UUID letterId, UUID playerUUID) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao marcar broadcast visto: " + e.getMessage());
        }
    }

    @Override
    public synchronized int countUnread(UUID playerUUID) {
        String sql = "SELECT COUNT(*) FROM letters WHERE recipient_uuid = ? AND read = 0 AND type NOT IN ('BROADCAST','JORNAL')";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, playerUUID.toString());
            ResultSet rs = ps.executeQuery();
    ...
    public synchronized List<Letter> getPendingLettersReadyToDeliver(long currentTime) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao buscar cartas prontas: " + e.getMessage());
            return Collections.emptyList();
        }
    }
    ...
    public synchronized boolean tryMarkDelivered(UUID letterId) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao marcar como entregue: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized void cleanup(long directOlderThan, long broadcastOlderThan, long auditOlderThan) {
        ...
        try {
            ...
            executeUpdate("DELETE FROM letters WHERE type NOT IN ('BROADCAST','JORNAL') AND read = 1 AND sent_at < ?", directOlderThan);
            ...
            logger.warning("[LetterForge] Erro durante cleanup: " + e.getMessage());
        }
    }
    ...
    public synchronized Optional<Letter> getLetterById(UUID letterId) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao buscar carta por ID: " + e.getMessage());
            return Optional.empty();
        }
    }
    ...
    public synchronized void revokeLetterById(UUID letterId) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao revogar carta: " + e.getMessage());
        }
    }
    ...
    public synchronized void logAudit(String eventType, String actorUuid, String actorName,
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.warning("[LetterForge] Erro ao gravar audit_log: " + e.getMessage());
        }
    }
    ...
    public synchronized List<AuditEntry> getRecentAuditLog(int limit) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao buscar audit_log: " + e.getMessage());
        }
        return list;
    }
    ...
    public synchronized void insertMapImage(UUID letterId, int tileIndex, int mapId,
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao salvar tile: " + e.getMessage());
        }
    }
    ...
    public synchronized void insertMapImageBatch(UUID letterId, int[] mapIds,
        ...
        try {
            ...
            logger.severe("[LetterForge] Erro ao salvar tiles em batch: " + e.getMessage());
        } finally {
    ...
    public synchronized List<MapImageEntry> getMapImagesForLetter(UUID letterId) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao buscar tiles: " + e.getMessage());
        }
        return list;
    }
    ...
    public synchronized List<MapImageEntry> getAllMapImages() {
        ...
        try (Statement s = connection.createStatement();
            ...
            logger.severe("[LetterForge] Erro ao carregar tiles: " + e.getMessage());
        }
        return list;
    }
    ...
    public synchronized void insertModerationImageBatch(UUID letterId, int[] mapIds, byte[][] tilesData,
        ...
        try {
            ...
            try { connection.rollback(); } catch (SQLException ignored) {}
            logger.severe("[LetterForge] Erro ao salvar imagem de moderacao: " + e.getMessage());
        } finally {
            try { connection.setAutoCommit(true); } catch (SQLException ignored) {}
        }
    }
    ...
    public synchronized List<MapImageEntry> getModerationImages(UUID letterId) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro ao buscar imagem de moderacao: " + e.getMessage());
        }
        return list;
    }
    ...
    public synchronized void cleanupModerationImages(long olderThan) {
        try (PreparedStatement ps = connection.prepareStatement(
            ...
            if (removed > 0)
                logger.info("[LetterForge] " + removed + " imagem(ns) de moderacao expirada(s) removida(s).");
        } catch (SQLException e) {
            logger.warning("[LetterForge] Erro ao limpar imagens de moderacao: " + e.getMessage());
        }
    }
    ...
    private void execute(String sql, String param) {
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.severe("[LetterForge] Erro SQL: " + e.getMessage());
        }
    }
